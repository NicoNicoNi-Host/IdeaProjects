    ...
    public static final SerialPort portParameterOpen(String portName, int baudrate) {
        try {  //通过端口名识别串口
            ...
            if (commPort instanceof SerialPort) {
                ...
                //设置baudrate 此处需要注意:波特率只能允许是int型 对于9600足够
                //8位数据位
                //1位停止位
                //无奇偶校验
...
//    1.1 自己写的方法，当端口可选时使用
/*    public static void action(String parameter,String port,int baudrate) throws Exception {
        // 打开串口
        SerialPort serialPort = portParameterOpen(port, baudrate);
        // 要发送的数据
        String dataSend = parameter;
    ...
    //    1.2 自己写的方法，新增获取点击状态
    public static String action(String parameter, String port, int baudrate) throws Exception {
        // 打开串口
        SerialPort serialPort = portParameterOpen(port, baudrate);
        // 要发送的数据
        String dataSend = parameter;
